...
## Live market data
V1 must use an authorized market-data feed/API for live or near-real-time prices. The implementation must not scrape Telda or rely on screenshots as a live feed.

Potential providers are treated as adapters, not as hard-coded truth. Before production use, verify API access rights, coverage, timestamp semantics, rate limits, redistribution rights, and symbol mapping.

Candidate research sources:
- EGX official platform: https://beta.egx.com.eg/
- EGX.news data services: https://egx.news/en/our-data
- ICE Egyptian Exchange data: https://developer.ice.com/fixed-income-data-services/catalog/egyptian-exchange-egx
...
- freshness exceeds configured threshold
